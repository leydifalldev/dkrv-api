...
const protoConfig = require("./src/gateway/gateway.json");
...
const generate = (config) => {
  config.targets.map(target => {
    ...
    return copyAndGenerateProto(config.src_folder, target.dest_folder, config.filename, target.go_filename);
  });
}
...
const copyAndGenerateProto = (proto_path_folder, dest_folder, filename, out_filename) => {
  const protoParams = {
    proto_path_folder,
  ...
  --go_out=plugins=grpc:<%= options.go_out_folder %> <%= options.proto_path_file %>', protoParams))
    ...
    .pipe(exec('protoc-go-inject-tag -XXX_skip=bson -input=<%= options.pb_go %>', {pb_go: dest_folder + out_filename}))
    .on('error', error => console.log(error.message));
}
...
function updateProto(cb) {
  watch([protoConfig.src_folder + protoConfig.filename], copyAndGenerateGatewayProto);
  cb();
}
